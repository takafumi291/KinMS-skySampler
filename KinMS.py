...
def makeHist(quant,sbMode,cloudIntensity,title):
    if sbMode == "skyProfile":
        print 'Plotting with weights'
        v,edge=np.histogram(quant,bins=63, density=True, weights = cloudIntensity) 
    else: v,edge=np.histogram(quant,bins=63, density=True) 
    plt.title(title)
    if (title != 'vRad'): plt.plot(edge[:-1],v,drawstyle='steps')
    else:plt.plot(edge[:-2],v[:-1],drawstyle='steps')
    plt.savefig('spectra.pdf',transparent=True)
    plt.show()
    plt.close()
    
def plotMap(xPos,yPos,quant,title = ''):
    ###############################################################
    #Place interesting quantity into array:
    import matplotlib.pyplot as plt
    outputArray = np.zeros([64,64])
    cellIndex = np.arange(0,64)
    cent=np.array([31,31])
    for i in np.arange(0,xPos.shape[0]):
        cubeX=int(xPos[i]+cent[0])
        cubeY=int(yPos[i]+cent[1])
        #print (cubeX,cubeY)
        if (0 < cubeX < 64) & (0 < cubeY < 64):
            #print 'Adding cloud to output cube'
            outputArray[cubeX,cubeY]=quant[i]
    #print np.min(outputArray)
    #print np.max(outputArray)
    #levels = np.linspace(np.min(outputArray),np.max(outputArray),20)
    levels = np.linspace(0.,50.,50.)
    #plt.contour(cellIndex,cellIndex,outputArray.T,levels=levels)
    plt.imshow(outputArray.T,cmap='jet',origin='lower')
    plt.title(title)
    plt.colorbar()
    plt.savefig('overlaySSvLos.pdf',transparent=True)
    plt.show()
    ################################################################
...
def kinms_create_velField_oneSided(velRad,velProf,r_flat,inc,posAng,gasSigma,seed,xPos,yPos,vPhaseCent=[0.0,0.0],vPosAng=False,vRadial=0.0,posAng_rad=0.0,inc_rad=0.0, sbMode = "axisymmetric",cloudIntensity=None):
    """
...
def KinMS(xs,ys,vs,cellSize,dv,beamSize,inc, sbMode = 'axisymmetric', samplerMode = 'list', 
    ...
    returnClouds=False,gasGrav=False,kinematic_Debug=False):
    """
    ...
    vPosAng: double or np.ndarray of double, optional
         (Default value = 0.0)
        Position angle of the kinematic major axis of the disc, relative to the position angle.
        Defined in the direction of the usual astronomical convention.
        In skyProfile mode, this is defined relative to the y-axis of the cube, following astronomical convention.
        Can be either a double, or an array of doubles. If single valued
        then the disc major axis is straight. If an array is passed then it should
        describe how the position angle changes as a function of `velrad`.
        Used to create position angle warps.
    ...
    """
    if sbMode == 'skyProfile':
        assert posAng == 0., "posAng should be 0 in skySampler mode - all rotation should be performed using vPosAng"
        assert phaseCen[0] == 0., "X-coordinate of phase centre should be 0 in skySampler mode - translations should be performed using vPhaseCen"
        assert phaseCen[1] == 0., "Y-coordinate of phase centre should be 0 in skySampler mode - translations should be performed using vPhaseCen"
    if(True in np.where(np.isnan(velProf), True, False)): print 'v NaNs exist'
    print velProf
    ...
    #Generate cloudlets using the sbMode selected by the user
    if sbMode == "axisymmetric":
        #Use sbRad and sbProf to generate nSamps from the distribution
        assert (len(sbRad) == len(sbProf)),"Shape of sbRad and sbProf do not match"
        #assert (len(sbRad) > 0), "Either sbRad or sbProf has length 0 - they must be specified in axisymmetric mode"
        assert (nSamps > 0), "Currently, nSamps is set to 0 or fewer - there must be some clouds used"
    ...
    if sbMode == "axisymmetric":
        ...
        #Need to project r_flat using the position angle and inclination to obtain the correct kinematic axes
        #Rotate to the major/minor axes
        #print 'Calculating los velocities in skyProfile mode'
        ang2Rot = 90 - vPosAng
        c = np.cos(np.radians(ang2Rot))
        s = np.sin(np.radians(ang2Rot))
        delX=xPos-np.full(xPos.shape,vPhaseCen[0])
        delY=yPos-np.full(yPos.shape,vPhaseCen[1])
    ...
    if len(vLOS_clouds):
        ...
        los_vel,coTan,vRad = kinms_create_velField_oneSided(velRad / cellSize,velProf,r_flat,inc,posAng,gasSigma,fixSeed,xPos,yPos,vPhaseCent=vPhaseCent,vPosAng=vPosAng,vRadial=vRadial,inc_rad=inc_rad,posAng_rad=posAng_rad, sbMode = sbMode, cloudIntensity = flux_clouds)
        if (sbMode != 'skyProfile'):
            #print '+++Applying rotations to the cloud positions+++'
    ...
    print x2_cent0
    print y2_cent1
    print los_vel_dv_cent2
    if(True in np.where(np.isnan(x2_cent0), True, False)): print 'x NaNs exist'
    if(True in np.where(np.isnan(y2_cent1), True, False)): print 'y NaNs exist'
    if(True in np.where(np.isnan(los_vel_dv_cent2), True, False)): print 'v NaNs exist'
    x2_cent0 = np.where(x2_cent0 == -0.,0.,x2_cent0)
    y2_cent1 = np.where(y2_cent1 == -0.,0.,y2_cent1)
    ...
    if nsubs > 0:
    	...
    	#print(nsubs)
    	print('Err: There are no clouds within the cube for these parameters')
    	cube = np.zeros([int(xSize),int(ySize),int(vSize)])
    	noClouds=True
    ...
    if noClouds == False:
        ...
        if not cleanOut:
            ...
            if sbMode == "skyProfile": 
               for i in range(0,cube.shape[2]): cube[:,:,i] = convolve_fft(cube[:,:,i], psf)
            else:
    ...
    plotMap(x2,y2,coTan,title='coTan')
    plotMap(x2,y2,vRad,title='vRad')
    plotMap(x2,y2,los_vel_dv_cent2,title='losVel')
    mp.makeplots(cube,xs,ys,vs,cellSize,dv,beamSize,posang=0,overcube=False,pvdthick=5,nconts=11., rms = 1.)
    ### EoT
    ...
    if returnClouds:
        retClouds = np.empty((inClouds.shape[0],3))
